  ...
  const rows = props.buttonsTab.map((buttonCategory, index) => {
    return (
      <button
        ...
        onClick={() => props.buttonCategory(buttonCategory, index)}>
...
const Main = (props) => {
  const mediaQueryButton = useMediaQuery('(width < 48em)');
  ...
  const buttonCategory = (buttonCategory, index) => {
    setbuttonState(buttonCategory, index);
  };
